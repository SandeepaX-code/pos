...
const navigation = [
  ...
  { label: "Dine-in", href: "/pos/dine-in", icon: Grid2x2, featured: true },
  { label: "Takeaway", href: "/pos/takeaway", icon: UtensilsCrossed },
  ...
  { label: "Settings", href: "/settings", icon: HeartPulse },
];
      ...
      <nav className="flex flex-1 flex-col gap-1 overflow-y-auto pr-1">
        {navigation.map((item) => (
          <a
            key={item.href}
            href={item.href}
            className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition ${item.featured ? "bg-orange-50 text-orange-700 shadow-sm" : "text-slate-700 hover:bg-orange-50 hover:text-orange-700"}`}
          >
