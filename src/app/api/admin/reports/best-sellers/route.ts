...
export const GET = requireAuth(async (req) => {
  ...
  try {
    ...
    const data = await service.getBestSellers(fromStr, toStr, limit, branchId);
    return jsonSuccess(data, 200, "Best selling products retrieved");
  } catch (e: unknown) {
    return jsonError(500, (e as Error).message || "Failed to retrieve best sellers", "INTERNAL_ERROR");
  }
});
