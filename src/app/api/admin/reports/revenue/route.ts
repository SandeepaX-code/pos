...
export const GET = requireAuth(async (req) => {
  ...
  try {
    ...
    const data = await service.getRevenueBreakdown(fromStr, toStr, branchId);
    return jsonSuccess(data, 200, "Revenue breakdown by payment method retrieved");
  } catch (e: unknown) {
    return jsonError(500, (e as Error).message || "Failed to retrieve revenue breakdown", "INTERNAL_ERROR");
  }
});
