...
export const GET = requireAuth(async (req) => {
  ...
  try {
    ...
    const data = await service.getCategoryPerformance(fromStr, toStr, branchId);
    return jsonSuccess(data, 200, "Category performance analytics retrieved");
  } catch (e: unknown) {
    return jsonError(500, (e as Error).message || "Failed to retrieve category performance", "INTERNAL_ERROR");
  }
});
